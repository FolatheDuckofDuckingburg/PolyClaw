...
Guide the user through creating a complete, high-quality PolyClaw plugin from initial concept to tested implementation. Follow a systematic approach: understand requirements, design components, clarify details, implement following best practices, validate, and test.
...
3. Create directory structure using bash:
   ```bash
   mkdir -p plugin-name/.polyclaw-plugin
...
6. Create .gitignore if needed (for .polyclaw/*.local.md, etc.)
...
2. For each command:
   ...
   - Write instructions FOR PolyClaw (not TO user)
...
2. Create .mcp.json configuration with:
   ...
   - Command and args (with ${POLYCLAW_PLUGIN_ROOT})
...
3. Create example .polyclaw/plugin-name.local.md file (as documentation)
4. Implement settings reading in hooks/commands as needed
5. Add to .gitignore: `.polyclaw/*.local.md`
...
5. **Test hook configuration** (if plugin has hooks):
   ...
   - Verify ${POLYCLAW_PLUGIN_ROOT} usage
...
**Goal**: Test that plugin works correctly in PolyClaw
...
1. **Installation instructions**:
   - Show user how to test locally:
     ...
     ```
   - Or copy to `.polyclaw-plugin/` for project testing
...
3. **Testing recommendations**:
   ...
   - For hooks: Use `polyclaw --debug` to see hook execution
...
- **Apply best practices**:
  ...
  - Commands written FOR PolyClaw
  - Strong trigger phrases
  - ${POLYCLAW_PLUGIN_ROOT} for portability
