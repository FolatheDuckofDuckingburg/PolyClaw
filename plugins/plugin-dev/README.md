...
A comprehensive toolkit for developing PolyClaw plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality PolyClaw plugins:
...
4. **Plugin Settings** - Configuration patterns using .polyclaw/plugin-name.local.md files
...
7. **Testing** - Verify plugin works in PolyClaw
...
**Trigger phrases:** "create a hook", "add a PreToolUse hook", "validate tool use", "implement prompt-based hooks", "${POLYCLAW_PLUGIN_ROOT}", "block dangerous commands"
...
- ${POLYCLAW_PLUGIN_ROOT} for portable paths
...
- Environment variable expansion (${POLYCLAW_PLUGIN_ROOT}, user vars)
...
- ${POLYCLAW_PLUGIN_ROOT} usage throughout
...
**What it covers:**
- .polyclaw/plugin-name.local.md pattern for configuration
...
- AI-assisted agent generation using PolyClaw's proven prompt
...
- 3 reference docs: agent-creation-system-prompt (from PolyClaw), system-prompt-design (~4,000w), triggering-examples (~2,500w)
...
- Based on skill-creator methodology adapted for PolyClaw plugins
...
Install from polyclaw-marketplace:

```bash
/plugin install plugin-dev@polyclaw-marketplace
```
...
This keeps PolyClaw's context focused while providing deep knowledge when needed.
...
- Based on official PolyClaw documentation
...
✅ **Portability**
- Use ${POLYCLAW_PLUGIN_ROOT} everywhere
- Relative paths only
...
This plugin is part of the polyclaw-marketplace. To contribute improvements:
