...
Practical examples of commands designed for PolyClaw plugins, demonstrating plugin-specific patterns and features.
...
!`node ${POLYCLAW_PLUGIN_ROOT}/scripts/quality-check.js $1`
...
**Key features:**
- Uses `${POLYCLAW_PLUGIN_ROOT}` for portable path
...
**Security scan:**
!`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/security-scan.sh $1`

**Performance analysis:**
!`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/perf-analyze.sh $1`

**Best practices check:**
!`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/best-practices.sh $1`
...
Template structure: @${POLYCLAW_PLUGIN_ROOT}/templates/api-documentation.md
...
!`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/pre-release-check.sh $1`

**Step 2 - Build artifacts:**
!`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/build-release.sh $1`

**Step 3 - Run test suite:**
!`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/run-tests.sh`

**Step 4 - Package release:**
!`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/package.sh $1`
...
Deployment configuration for $1: @${POLYCLAW_PLUGIN_ROOT}/config/$1-deploy.json
...
- Plugin's linting rules: ${POLYCLAW_PLUGIN_ROOT}/config/lint-rules.json
- Security checklist: ${POLYCLAW_PLUGIN_ROOT}/checklists/security.md
- Performance guidelines: ${POLYCLAW_PLUGIN_ROOT}/docs/performance.md
...
Run plugin analyzer: !`node ${POLYCLAW_PLUGIN_ROOT}/scripts/analyze.js $1`
...
Template: @${POLYCLAW_PLUGIN_ROOT}/templates/review-report.md
...
Check build script exists: !`test -x ${POLYCLAW_PLUGIN_ROOT}/scripts/build.sh && echo "EXISTS" || echo "MISSING"`

Verify configuration available: !`test -f ${POLYCLAW_PLUGIN_ROOT}/config/$1.json && echo "FOUND" || echo "NOT_FOUND"`

If all validations pass:

**Configuration:** @${POLYCLAW_PLUGIN_ROOT}/config/$1.json

**Execute build:** !`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/build.sh $1 2>&1`

**Validation results:** !`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/validate-build.sh $1 2>&1`
...
Load environment configuration: @${POLYCLAW_PLUGIN_ROOT}/config/$1-checks.json

Determine check level: !`echo "$1" | grep -E "^prod$" && echo "FULL" || echo "BASIC"`

**For production environment:**
- Full test suite: !`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/test-full.sh`
- Security scan: !`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/security-scan.sh`
- Performance audit: !`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/perf-check.sh`
- Compliance check: !`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/compliance.sh`

**For non-production environments:**
- Basic tests: !`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/test-basic.sh`
- Quick lint: !`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/lint.sh`
...
```markdown
Check exists: !`test -f ${POLYCLAW_PLUGIN_ROOT}/path/file && echo "YES" || echo "NO"`
```
...
1. **Test with plugin installed:**
   ```bash
   cd /path/to/plugin
   polyclaw /command-name args
   ```

2. **Verify ${POLYCLAW_PLUGIN_ROOT} expansion:**
   ```bash
   # Add debug output to command
   !`echo "Plugin root: ${POLYCLAW_PLUGIN_ROOT}"`
   ```

3. **Test across different working directories:**
   ```bash
   cd /tmp && polyclaw /command-name
   cd /other/project && polyclaw /command-name
   ```

4. **Validate resource availability:**
   ```bash
   # Check all plugin resources exist
   !`ls -la ${POLYCLAW_PLUGIN_ROOT}/scripts/`
   !`ls -la ${POLYCLAW_PLUGIN_ROOT}/config/`
   ```

### Common Mistakes to Avoid

1. **Using relative paths instead of ${POLYCLAW_PLUGIN_ROOT}:**
   ```markdown
   # Wrong
   !`node ./scripts/analyze.js`

   # Correct
   !`node ${POLYCLAW_PLUGIN_ROOT}/scripts/analyze.js`
   ```
...
2. **Forgetting to allow required tools:**
   ...
   allowed-tools: Bash(*)
   ---
   !`bash ${POLYCLAW_PLUGIN_ROOT}/scripts/script.sh`
   ```
...
4. **Hardcoding plugin paths:**
   ...
   @/home/user/.polyclaw/plugins/my-plugin/config.json

   # Correct - works everywhere
   @${POLYCLAW_PLUGIN_ROOT}/config.json
   ```
