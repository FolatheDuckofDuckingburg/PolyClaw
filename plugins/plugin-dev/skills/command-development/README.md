...
Comprehensive guidance on creating PolyClaw slash commands, including file format, frontmatter options, dynamic arguments, and best practices.
...
- Plugin-specific features (${POLYCLAW_PLUGIN_ROOT}, plugin patterns)
...
- **plugin-features-reference.md**: Plugin-specific command features
  - Plugin command discovery and organization
  - ${POLYCLAW_PLUGIN_ROOT} environment variable usage
  - Plugin command patterns (configuration, template, multi-script)
...
PolyClaw activates this skill when users:
...
PolyClaw loads references and examples as needed based on task.
...
- **Project**: `.polyclaw/commands/` (shared with team)
- **Personal**: `~/.polyclaw/commands/` (your commands)
...
- ✓ Plugin command patterns (${POLYCLAW_PLUGIN_ROOT}, discovery, organization)
...
6. Test examples work with current PolyClaw
