...
head -n 20 .polyclaw/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .polyclaw/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .polyclaw/commands/*.md

# Check file is in correct location
test -f .polyclaw/commands/my-command.md && echo "Found" || echo "Missing"
```
...
# 5. Check debug logs
tail -f ~/.polyclaw/debug-logs/latest
...
cat > .polyclaw/commands/test-bash.md << 'EOF'
---
...
cat > .polyclaw/commands/test-forbidden.md << 'EOF'
---
...
# Command: .polyclaw/commands/risky-operation.md
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.polyclaw/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Validate command structure
        run: |
          for cmd in .polyclaw/commands/*.md; do
            echo "Testing: $cmd"
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .polyclaw/commands/; then
...
```bash
# Monitor PolyClaw during command execution
# In terminal 1:
polyclaw --debug

# In terminal 2:
watch -n 1 'ps aux | grep polyclaw'
...
```bash
# Check file location
ls -la .polyclaw/commands/my-command.md

# Check permissions
chmod 644 .polyclaw/commands/my-command.md

# Check syntax
head -n 20 .polyclaw/commands/my-command.md
...
```bash
# Verify syntax
grep '\$1' .polyclaw/commands/my-command.md
grep '\$ARGUMENTS' .polyclaw/commands/my-command.md

# Test with simple command first
echo "Test: \$1 and \$2" > .polyclaw/commands/test-args.md
```
...
```bash
# Check allowed-tools
grep "allowed-tools" .polyclaw/commands/my-command.md

# Verify command syntax
grep '!\`' .polyclaw/commands/my-command.md
