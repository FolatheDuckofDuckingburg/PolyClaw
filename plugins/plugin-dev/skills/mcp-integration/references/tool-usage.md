...
Complete guide to using MCP tools effectively in PolyClaw plugin commands and agents.

## Overview

Once an MCP server is configured, its tools become available with the prefix `mcp__plugin_<plugin-name>_<server-name>__<tool-name>`. Use these tools in commands and agents just like built-in PolyClaw tools.
...
- Can use any tool PolyClaw determines is necessary
...
PolyClaw automatically structures tool calls based on schema:

```typescript
// PolyClaw generates this internally
{
...
```markdown
Steps:
1. Make parallel calls (PolyClaw handles this automatically):
...
2. **Install plugin locally** in `.polyclaw-plugin/`
3. **Verify tools available** with `/mcp`
4. **Test command** that uses tools
5. **Check debug output**: `polyclaw --debug`
