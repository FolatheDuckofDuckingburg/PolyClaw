...
Complete reference for all MCP server types supported in PolyClaw plugins.
...
{
  "my-server": {
    "command": "${POLYCLAW_PLUGIN_ROOT}/servers/custom-server",
...
1. **Startup**: PolyClaw spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire PolyClaw session
4. **Shutdown**: Process terminated when PolyClaw exits
...
{
  "custom": {
    "command": "${POLYCLAW_PLUGIN_ROOT}/servers/my-server.js",
    "args": ["--verbose"]
  }
}
```
...
1. **Use absolute paths or ${POLYCLAW_PLUGIN_ROOT}**
