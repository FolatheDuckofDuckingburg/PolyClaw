...
Complete guide to authentication methods for MCP servers in PolyClaw plugins.
...
PolyClaw automatically handles the complete OAuth 2.0 flow for SSE and HTTP servers:

1. User attempts to use MCP tool
2. PolyClaw detects authentication needed
3. Opens browser for OAuth consent
4. User authorizes in browser
5. Tokens stored securely by PolyClaw
...
No additional auth configuration needed! PolyClaw handles everything.
...
Tokens are stored securely by PolyClaw:
...
```json
{
  "api": {
    ...
    "headersHelper": "${POLYCLAW_PLUGIN_ROOT}/scripts/get-headers.sh"
  }
}
```
...
{
  "secure-api": {
    "command": "${POLYCLAW_PLUGIN_ROOT}/servers/mtls-wrapper",
...
```json
{
  "headersHelper": "${POLYCLAW_PLUGIN_ROOT}/scripts/generate-jwt.sh"
}
```
