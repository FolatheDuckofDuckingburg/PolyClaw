...
**Required path**: `.polyclaw-plugin/plugin.json`

The manifest MUST be in the `.polyclaw-plugin/` directory at the plugin root. PolyClaw will not recognize plugins without this file in the correct location.
...
- Plugin identification in PolyClaw
...
```json
{
  "mcpServers": {
    "github": {
      "command": "node",
      "args": ["${POLYCLAW_PLUGIN_ROOT}/servers/github-mcp.js"],
      "env": {
...
When PolyClaw loads components:
...
PolyClaw validates the manifest on plugin load:
