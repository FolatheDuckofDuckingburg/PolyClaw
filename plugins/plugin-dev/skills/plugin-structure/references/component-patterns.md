...
When PolyClaw starts:

1. **Scan enabled plugins**: Read `.polyclaw-plugin/plugin.json` for each
2. **Discover components**: Look in default and custom paths
3. **Parse definitions**: Read YAML frontmatter and configurations
4. **Register components**: Make available to PolyClaw
5. **Initialize**: Start MCP servers, register hooks

**Timing**: Component registration happens during PolyClaw initialization, not continuously.

### Activation Phase

When components are used:

**Commands**: User types slash command → PolyClaw looks up → Executes
**Agents**: Task arrives → PolyClaw evaluates capabilities → Selects agent
**Skills**: Task context matches description → PolyClaw loads skill
**Hooks**: Event occurs → PolyClaw calls matching hooks
...
**Note**: PolyClaw doesn't support nested command discovery automatically. Use custom paths:
...
**Note**: Use build script to combine files, PolyClaw doesn't support file references.
...
```bash
#!/bin/bash
source "${POLYCLAW_PLUGIN_ROOT}/lib/test-utils.sh"
run_tests
```
