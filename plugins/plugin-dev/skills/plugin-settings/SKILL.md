...
description: This skill should be used when the user asks about "plugin settings", "store plugin configuration", "user-configurable plugin", ".local.md files", "plugin state files", "read YAML frontmatter", "per-project plugin settings", or wants to make plugin behavior configurable. Documents the .polyclaw/plugin-name.local.md pattern for storing plugin-specific configuration with YAML frontmatter and markdown content.
version: 0.1.0
---

# Plugin Settings Pattern for PolyClaw Plugins

## Overview

Plugins can store user-configurable settings and state in `.polyclaw/plugin-name.local.md` files within the project directory. This pattern uses YAML frontmatter for structured configuration and markdown content for prompts or additional context.

**Key characteristics:**
- File location: `.polyclaw/plugin-name.local.md` in project root
...
- Prompts to feed back to PolyClaw
- Documentation or notes
```

### Example: Plugin State File

**.polyclaw/my-plugin.local.md:**
```markdown
---
enabled: true
strict_mode: false
...
STATE_FILE=".polyclaw/my-plugin.local.md"
...
Steps:
1. Check if settings exist at `.polyclaw/my-plugin.local.md`
...
Check for plugin settings at `.polyclaw/my-plugin.local.md`.
...
```bash
#!/bin/bash
STATE_FILE=".polyclaw/security-scan.local.md"
...
**.polyclaw/multi-agent-swarm.local.md:**
```markdown
---
...
2. Create `.polyclaw/my-plugin.local.md` with YAML frontmatter
3. Set appropriate values based on user input
4. Inform user that settings are saved
5. Remind user to restart PolyClaw for hooks to recognize changes
```
...
Create `.polyclaw/my-plugin.local.md` in your project:
...
After creating or editing, restart PolyClaw for changes to take effect.
```
...
✅ **DO:**
- Use `.polyclaw/plugin-name.local.md` format
- Match plugin name exactly
- Use `.local.md` suffix for user-local files

❌ **DON'T:**
- Use different directory (not `.polyclaw/`)
...
**Important:** Settings changes require PolyClaw restart.
...
After editing `.polyclaw/my-plugin.local.md`:
1. Save the file
2. Exit PolyClaw
...
```bash
if [[ ! -f ".polyclaw/my-plugin.local.md" ]]; then
...
3. Add gitignore entry for `.polyclaw/*.local.md`
...
7. Remind users that changes require PolyClaw restart
