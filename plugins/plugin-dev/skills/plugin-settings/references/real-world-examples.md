...
Detailed analysis of how production plugins use the `.polyclaw/plugin-name.local.md` pattern.
...
SWARM_STATE_FILE=".polyclaw/multi-agent-swarm.local.md"
...
```bash
cat > "$WORKTREE_PATH/.polyclaw/multi-agent-swarm.local.md" <<EOF
---
...
sed "s/^pr_number: .*/pr_number: $PR_NUM/" \
  ".polyclaw/multi-agent-swarm.local.md" > temp.md
mv temp.md ".polyclaw/multi-agent-swarm.local.md"
```
...
Document any changes needed in POLYCLAW.md.
```
...
**Purpose:** Prevent session exit and loop PolyClaw's output back as input
...
RALPH_STATE_FILE=".polyclaw/ralph-loop.local.md"
...
# Create state file
cat > ".polyclaw/ralph-loop.local.md" <<EOF
---
iteration: 1
...
echo "Ralph loop initialized: .polyclaw/ralph-loop.local.md"
```
...
|---------|-------------------|--------------|
| **File** | `.polyclaw/multi-agent-swarm.local.md` | `.polyclaw/ralph-loop.local.md` |
...
```bash
# BAD
FILE="/Users/alice/.polyclaw/my-plugin.local.md"

# GOOD
FILE=".polyclaw/my-plugin.local.md"
```
...
The `.polyclaw/plugin-name.local.md` pattern provides:
