...
This reference provides common, proven patterns for implementing PolyClaw hooks. Use these patterns as starting points for typical hook use cases.
  ...
  "Notification": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${POLYCLAW_PLUGIN_ROOT}/scripts/log-notification.sh"
        }
      ]
    }
  ]
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${POLYCLAW_PLUGIN_ROOT}/scripts/check-quality.sh"
        }
      ]
    }
  ]
...
FLAG_FILE="$POLYCLAW_PROJECT_DIR/.enable-security-scan"
...
**Note:** Must restart PolyClaw after creating/removing flag files for hooks to recognize changes.
...
```bash
#!/bin/bash
CONFIG_FILE="$POLYCLAW_PROJECT_DIR/.polyclaw/my-plugin.local.json"
...
**Configuration file (.polyclaw/my-plugin.local.json):**
```json
{
  "strictMode": true,
  "maxFileSize": 500000,
