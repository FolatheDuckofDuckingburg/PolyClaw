  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${POLYCLAW_PLUGIN_ROOT}/scripts/quick-check.sh",
          "timeout": 5
        },
        {
          "type": "prompt",
...
if [ -f ".polyclaw-hooks-config.json" ]; then
  strict_mode=$(jq -r '.strict_mode' .polyclaw-hooks-config.json)
...
**Example .polyclaw-hooks-config.json:**
```json
{
  "strict_mode": true,
...
# Append to audit log
echo "$timestamp | $USER | $tool_name | $input" >> ~/.polyclaw/audit.log
...
export POLYCLAW_PROJECT_DIR="/tmp/test-project"
export POLYCLAW_PLUGIN_ROOT="$(pwd)"
mkdir -p "$POLYCLAW_PROJECT_DIR"
