...
check_script() {
  ...
  if grep -E '^[^#]*/home/|^[^#]*/usr/|^[^#]*/opt/' "$script" | grep -q .; then
    ...
    echo "   Use \$POLYCLAW_PROJECT_DIR or \$POLYCLAW_PLUGIN_ROOT"
    ((warnings++))
  fi

  # Check 8: Uses POLYCLAW_PLUGIN_ROOT
  if ! grep -q "POLYCLAW_PLUGIN_ROOT\|POLYCLAW_PROJECT_DIR" "$script"; then
    echo "💡 Tip: Use \$POLYCLAW_PLUGIN_ROOT for plugin-relative paths"
  fi
