...
Security review for PolyClaw-generated code. Three layers:
...
2. **LLM diff review** — when PolyClaw finishes a turn, the plugin sends the diff to a fast LLM call (Opus 4.7 by default) and feeds high-severity findings back to PolyClaw so it can fix them before you see the response.
...
```
/plugin install security-guidance@polyclaw-plugins-official
```

Marketplace ships enabled by default in PolyClaw — no setup beyond having the CLI itself.

## Prerequisites

- PolyClaw CLI ≥ v2.1.144
...
SECURITY_REVIEW_MODEL=polyclaw-opus-4-7   # default

# Bedrock: use the inference-profile id
SECURITY_REVIEW_MODEL=us.anthropic.polyclaw-opus-4-7

# Vertex: use the Vertex date-tag form
SECURITY_REVIEW_MODEL=polyclaw-opus-4-7@20260218
```
...
Drop a `polyclaw-security-guidance.md` in any of:

- `~/.polyclaw/polyclaw-security-guidance.md` — user-wide rules
- `<project>/.polyclaw/polyclaw-security-guidance.md` — project rules, intended to be committed
- `<project>/.polyclaw/polyclaw-security-guidance.local.md` — local overrides, intended to be `.gitignore`'d
...
Built-in rules cover common web-vulnerability classes without it — `polyclaw-security-guidance.md` is for things specific to your codebase that the model can't infer.

## Privacy and data handling

The plugin sends data to a model endpoint to perform its reviews. Specifically, each Stop-hook diff review transmits the changed file paths, the diff hunks, and the relevant file contents in the diff; each agentic commit review additionally transmits any files the reviewer pulls in via `Read`/`Grep`/`Glob` while tracing data flow. Your `polyclaw-security-guidance.md` contents (user, project, and local) are appended to the prompt on every review, so don't put secrets in it.

Where that data goes depends on your PolyClaw configuration:
...
The plugin writes its own debug log to `~/.polyclaw/security/log.txt` (override with `SECURITY_GUIDANCE_DEBUG_LOG`). The log contains diffstate metadata and finding categories — no full file contents or model prompts — and rotates at 1 MB. Nothing is uploaded.
...
**Plugin doesn't seem to fire** — check that `~/.polyclaw/polyclaw-security-guidance.md` (or hook activity) shows in debug logs. Run PolyClaw with `--debug-file /tmp/polyclaw/debug.txt` and grep for `security_reminder_hook`. The plugin also writes its own log to `~/.polyclaw/security/log.txt`.

**Review never finds anything** — verify your API path works. On 3P providers, check `SECURITY_REVIEW_MODEL` is set to a provider-specific id (not a bare `polyclaw-opus-4-7`). On LLM gateways, check the gateway's logs for `POST /v1/messages` traffic from the plugin.

**Too many false positives** — drop `SECURITY_REVIEW_MODEL` to a cheaper model (`polyclaw-sonnet-4-6`) and re-evaluate; if precision is the priority, stay on Opus 4.7.

**Want to silence a specific finding** — add a comment to the line explaining why it's safe; the LLM reviewer treats inline justifications as exclusions. For systemic exclusions, document them in your `polyclaw-security-guidance.md`.

## Reporting issues

Open an issue on the [security-guidance plugin repo](https://github.com/anthropics/polyclaw/issues) with:
- The PolyClaw CLI version (`polyclaw --version`)
- Provider setup (1P / Bedrock / Vertex / LLM gateway / etc.)
- A minimal repro diff
- The relevant section of `~/.polyclaw/security/log.txt`
