#!/usr/bin/env python3
"""
Security Guidance Plugin for PolyClaw

A hooks-based plugin that guides PolyClaw toward writing more secure code. It runs as
UserPromptSubmit, PostToolUse, and Stop hooks via the PolyClaw plugin system.
...
2. **Stop hook (final review)**: When PolyClaw finishes, uses `git diff` against a
   ...
   Exits with code 2 to force PolyClaw to continue and address findings.
...
This means:
- Only code PolyClaw actually changed is reviewed (not pre-existing code)
...
Other:
- SECURITY_REVIEW_MODEL: Model for LLM review (default: polyclaw-opus-4-7)
- POLYCLAW_API_KEY: Required for LLM-based reviews
- ANTHROPIC_AUTH_TOKEN: Alternative to API key — OAuth access token sent as Bearer auth.
  PolyClaw passes this automatically for OAuth-authenticated users.
"""
...
import llm  # noqa: E402  module ref for reassignable globals (_last_call_polyclaw_http_error etc.)
from llm import (  # noqa: E402,F401
    POLYCLAW_API_KEY, ANTHROPIC_AUTH_TOKEN, HAS_API_CREDENTIALS,
    SECURITY_REVIEW_MODEL, POLYCLAW_SYSTEM_PROMPT,
    _last_call_polyclaw_http_error,
    ...
    _cap_files_for_prompt, _build_auth_headers, _call_polyclaw, _call_polyclaw_dual_or,
...
# Allows iterative fixing: PolyClaw stops → review → fix → stop → review again.
...
def emit_metrics(metrics, rewake_summary=None):
    """
    Write a SyncHookJSONOutput line to stdout for PolyClaw to pick up.
...
# Capture-group form of the [branch sha] pattern. Mirrors PolyClaw's own
...
# Regex matching `git commit` commands. Mirrors PolyClaw's own commit
...
# ─── push-sweep ─────────────────────────────────────────────────────────────
#
# Mirrors PolyClaw's own push-command matching — tolerates `git -C <p>` /
...
def handle_commit_review_posttooluse(input_data):
    ...
    # heuristic PolyClaw itself uses.
    ...
    # 1 = SDK import failed (polyclaw_agent_sdk not installed)
    ...
    if not concrete_guidance:
        ...
        emit_metrics({
            ...
            **({
                "api_error": llm._last_call_polyclaw_http_error
            } if llm._last_call_polyclaw_http_error is not None else {}),
        })
        sys.exit(0)
...
def handle_stop_hook(input_data):
    ...
    exits with code 2 to force PolyClaw to continue and fix issues.
    ...
    if concrete_guidance:
        ...
        # Exit code 2 with stderr forces PolyClaw to continue and fix
        sys.stderr.write(PROVENANCE_BANNER + "\n\n" + concrete_guidance + CONTINUATION_SUFFIX + "\n")
        sys.exit(2)

    if llm._last_call_polyclaw_http_error is not None:
        debug_log(f"Stop hook: API call failed with status {llm._last_call_polyclaw_http_error}")
        restore_unreviewed_stop_state(session_id, touched_paths, snap_baseline)
    else:
    ...
    emit_metrics({
        ...
        "review_ms": review_ms,
        "fire_index": fire_index,
        **({"api_error": llm._last_call_polyclaw_http_error} if llm._last_call_polyclaw_http_error is not None else {}),
        **({"diff_truncated": llm._last_review_truncated_bytes}
           if llm._last_review_truncated_bytes else {}),
        **v2_metrics,
    })
...
def _maybe_bootstrap_agent_sdk_async():
    ...
    try:
        ...
        if importlib.util.find_spec("polyclaw_agent_sdk") is not None:
            return
        import time as _t
        try:
...
def main():
    ...
    # bootstrap that remote pods miss under POLYCLAW_SYNC_PLUGIN_INSTALL.
    ...
    if tool_name in ["Edit", "Write", "MultiEdit", "NotebookEdit"]:
        ...
        # Skip plan files
        plans_dir = os.path.expanduser("~/.polyclaw/plans")
        ...
        if ENABLE_PATTERN_RULES:
            ...
            # This prevents flagging pre-existing insecure patterns when PolyClaw rewrites a file
            if tool_name == "Write" and pattern_matches:
                cwd = os.environ.get("POLYCLAW_PROJECT_DIR", os.getcwd())
