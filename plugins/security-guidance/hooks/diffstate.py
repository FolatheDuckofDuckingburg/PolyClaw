...
def consume_stop_state(session_id):
    ...
    The Stop hook is asyncRewake — it runs in the background after PolyClaw's
...
def capture_git_baseline(cwd):
    ...
    files are not reviewed as PolyClaw-authored.
    """
    try:
...
# `.git/polyclaw-trailers`) so it survives across sessions and is per-clone.
#
