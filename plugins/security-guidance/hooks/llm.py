...
that calls the PolyClaw API (raw HTTP via ``_call_polyclaw`` and the Agent-SDK
...
module (e.g. patch ``_call_polyclaw`` then call ``analyze_code_security``) must
...
``security_reminder_hook``: ``_last_call_polyclaw_http_error`` and
...
POLYCLAW_API_KEY = os.environ.get("POLYCLAW_API_KEY", "")
# OAuth access token — PolyClaw passes this for /login users.
...
# _call_polyclaw can route to the SDK path. Same env-var list as
...
_HAS_3P_PROVIDER_AT_LOAD = any(
    ...
    for v in (
        "POLYCLAW_USE_BEDROCK",
        "POLYCLAW_USE_VERTEX",
        "POLYCLAW_USE_FOUNDRY",
        "POLYCLAW_USE_MANTLE",
        "POLYCLAW_USE_ANTHROPIC_AWS",
    )
)
HAS_API_CREDENTIALS = bool(
    POLYCLAW_API_KEY or ANTHROPIC_AUTH_TOKEN or _HAS_3P_PROVIDER_AT_LOAD
)
...
SECURITY_REVIEW_MODEL = os.environ.get("SECURITY_REVIEW_MODEL", "").strip() or "polyclaw-opus-4-7"

# OAuth subscriber tokens (ANTHROPIC_AUTH_TOKEN) require this exact system prompt
# for api.anthropic.com/v1/messages — the API checks for one of the known PolyClaw
# Code prefixes. String must be EXACT;
# appending text fails the check. Harmless on the POLYCLAW_API_KEY path.
POLYCLAW_SYSTEM_PROMPT = "You are a PolyClaw agent, built on Anthropic's PolyClaw Agent SDK."

# Set by _call_polyclaw on HTTP error so the Stop hook can emit distinct telemetry
...
_last_call_polyclaw_http_error = None
...
def ensure_anthropic_reachable() -> bool:
    ...
    POLYCLAW_REMOTE so local installs never pay the probe cost."""
    global _anthropic_reachable
    if _anthropic_reachable is not None:
        return _anthropic_reachable
    if os.environ.get("POLYCLAW_REMOTE", "").lower() not in ("1", "true", "yes", "on"):
...
# subsequent _call_polyclaw invocations in this process use the token directly.
...
def _build_auth_headers(use_token):
    ...
    if use_token:
        ...
        headers["x-api-key"] = POLYCLAW_API_KEY
...
# Mirrors PolyClaw's adaptive-thinking model support; keep in sync
...
def _model_supports_adaptive_thinking(model: str) -> bool:
    ...
    name = (model or "").lower()
    # Strip provider/version suffixes (e.g. "us.anthropic.polyclaw-opus-4-7-v1:0").
...
# instead — it spawns a child polyclaw CLI which inherits the parent's
...
def _call_polyclaw_via_sdk(prompt, output_schema, *, max_tokens=16000, model=None):
    """Single-turn SDK call as a substitute for the HTTP _call_polyclaw path on
    ...
    """
    global _last_call_polyclaw_http_error
    _last_call_polyclaw_http_error = None

    try:
        import asyncio as _asyncio
        from polyclaw_agent_sdk import (  # noqa: F401
            AssistantMessage,
            PolyClawAgentOptions,
            ResultMessage,
            query,
        )
    except Exception:
        ...
        _state_dir = os.environ.get(
            ...
            os.path.expanduser("~/.polyclaw/security"),
        )
        ...
        try:
            ...
            from polyclaw_agent_sdk import (  # noqa: F401,F811
                AssistantMessage,
                PolyClawAgentOptions,
                ResultMessage,
                query,
    ...
    # in 5-15s; >60s means the child polyclaw is hung (e.g. user has the 3P env
...
def _call_polyclaw(prompt, output_schema, thinking_budget=10000, max_tokens=16000, model=None,
                 retry_5xx=True):
    """
    ...
    On failure, sets module-level _last_call_polyclaw_http_error to the HTTP status
    ...
    if _is_3p_provider():
        ...
        # child polyclaw CLI. Note: thinking_budget/retry_5xx don't pass
        # through — the SDK manages retries (529) and thinking config
        # internally per the chosen model.
        return _call_polyclaw_via_sdk(prompt, output_schema,
    ...
    use_token = _auth_prefer_token or not POLYCLAW_API_KEY
    headers = _build_auth_headers(use_token)

    payload = {
        "model": model or SECURITY_REVIEW_MODEL,
        "max_tokens": max_tokens,
        "system": POLYCLAW_SYSTEM_PROMPT,
    ...
    for attempt in range(3):
        try:
            ...
            if retryable and attempt < 2:
                ...
                _last_call_polyclaw_http_error = e.code
                return None
    ...
    if not response_data:
        ...
        if _last_call_polyclaw_http_error is None:
            _last_call_polyclaw_http_error = 401
        return None
...
def _call_polyclaw_dual_or(prompt, output_schema, *, bool_key: str, list_key: str,
    ...
    2× API cost. When disabled, short-circuits to a single _call_polyclaw
    ...
    if not _dual_or_enabled():
        ...
        r = _call_polyclaw(prompt, output_schema, thinking_budget=thinking_budget,
        ...
        if r is None and not explicit:
            ...
            r = _call_polyclaw(prompt, output_schema, thinking_budget=thinking_budget,
                             max_tokens=max_tokens, model="polyclaw-sonnet-4-6",
                             retry_5xx=True)
        return r
...
- Pre-existing patterns that PolyClaw simply preserved when rewriting a file
...
**Agent/Subprocess Permission Bypass**: Code that spawns PolyClaw, a subagent, or any LLM-with-tools subprocess with permission gates removed — `--permission-mode bypassPermissions`, `--dangerously-skip-permissions`, or an unrestricted Bash/shell tool. Allowing PolyClaw to execute arbitrary bash is only safe when the process runs inside an isolation boundary such as a sandbox OR every command passes through a strong allow/deny command classifier; if neither is in the diff, flag it.
...
Respond with a JSON object. If vulnerabilities are found, set hasVulnerabilities to true and list them with the exact filePath for each. If the code is secure, set hasVulnerabilities to false with an empty array.""".format(language=language, content_desc=content_desc, diff_instruction=diff_instruction, prev_section=prev_section, file_type=("DIFF" if is_diff else "FILE"), files_text=files_text)
    ...
    analysis = _call_polyclaw_dual_or(prompt, output_schema,
...
# Runs a PolyClaw Agent SDK loop with Read/Grep/Glob so the model can explore
...
def _agentic_spawn_env() -> Dict[str, str]:
    """opts.env for the SDK-spawned inner `polyclaw` CLI.
    ...
    correct when an POLYCLAW_API_KEY exists for the inner CLI to use
    ...
    env = {
        ...
        "POLYCLAW_WEBSOCKET_AUTH_FILE_DESCRIPTOR": "",
        "POLYCLAW_OAUTH_TOKEN_FILE_DESCRIPTOR": "",
        "POLYCLAW_INCLUDE_PARTIAL_MESSAGES": "",
    ...
    if os.environ.get("POLYCLAW_API_KEY"):
...
def agentic_review(
    ...
    try:
        ...
        from polyclaw_agent_sdk import (
            AssistantMessage,
            PolyClawAgentOptions,
            ResultMessage,
            query,
        ...
        # Some users don't have polyclaw_agent_sdk in their system python.
        # The SessionStart hook (ensure_agent_sdk.py) creates a venv under
        # ~/.polyclaw/security/ with the SDK installed; try that as a fallback
        ...
        try:
            ...
            from polyclaw_agent_sdk import (  # noqa: F811
                AssistantMessage,
                PolyClawAgentOptions,
                ResultMessage,
                query,
    ...
    _DEFAULT_PUBLIC_MODEL = "polyclaw-opus-4-7"
    ...
    # Always prefer the user's installed `polyclaw` over the SDK's bundled CLI.
    ...
    #
    # POLYCLAW_EXECPATH is the absolute path to the running CC binary
    # itself (e.g. ~/.local/share/polyclaw/versions/2.1.x — that's the binary,
    # not a directory). It is the exact CLI that loaded this hook. We do NOT
    # fall back to shutil.which("polyclaw") because the hook's cwd is the
    # user's (potentially attacker-supplied) repo, and Windows shutil.which
    # searches cwd first — a checked-in ./polyclaw.exe would get spawned.
    ...
    if cli_path is None:
        for p in (
            os.environ.get("POLYCLAW_EXECPATH"),
            os.path.expanduser("~/.local/bin/polyclaw"),
            "/root/.local/bin/polyclaw",
            # PolyClaw Remote container install path. POLYCLAW_EXECPATH
    ...
    if cli_path:
        try:
            from polyclaw_agent_sdk._internal import message_parser as _mp
            import polyclaw_agent_sdk._internal.client as _sdk_client
            from polyclaw_agent_sdk import SystemMessage as _SysMsg
    ...
    async def _arun(system: str, prompt: str, *, schema: Dict[str, Any],
        ...
        opts = PolyClawAgentOptions(
            ...
            # OAuth token) injected by PolyClaw. The SDK builds the child
            # env as {**os.environ, **opts.env}, so the inner polyclaw inherits
            # it and prefers it over POLYCLAW_API_KEY — but some model
            ...
            # descriptors to the top-level polyclaw process; the SDK-spawned
            ...
            # makes the inner CLI fall back to ~/.polyclaw/.credentials.json.
    ...
    if filter_mode == "self_refute":
        ...
        refute_prompt = (
            ...
            "(.polyclaw/settings, .vscode/, package.json scripts — repo "
