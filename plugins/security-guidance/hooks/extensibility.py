...
1. ``polyclaw-security-guidance.md`` — markdown appended to every LLM review prompt.
...
Discovery, in precedence order (matching POLYCLAW.md / settings.json):
  - ``~/.polyclaw/<name>``                  (user)
  - ``<cwd>/.polyclaw/<name>``              (project, committed)
  - ``<cwd>/.polyclaw/<name>.local.<ext>``  (project local, gitignored)

Managed delivery via ``managed-settings.json`` is not yet supported.
Org admins can still push files to ``~/.polyclaw/`` via MDM/GPO.
...
GUIDANCE_BASENAME = "polyclaw-security-guidance.md"
...
def load_for_session(cwd: Optional[str]) -> None:
    ...
    try:
        ...
        debug_log(f"extensibility: failed to load polyclaw-security-guidance.md: {e}")
        _guidance_block = ""
    try:
...
def _config_paths(cwd: Optional[str], basename: str) -> List[Tuple[str, str]]:
    ...
    paths = [("User", os.path.expanduser(os.path.join("~", ".polyclaw", basename)))]
    if cwd:
        paths.append(("Project", os.path.join(cwd, ".polyclaw", basename)))
        # polyclaw-security-guidance.local.md / security-patterns.local.yaml
        stem, ext = os.path.splitext(basename)
        paths.append(("Project (local)", os.path.join(cwd, ".polyclaw", f"{stem}.local{ext}")))
    return paths
...
def _load_guidance(cwd: Optional[str]) -> str:
    ...
    if len(combined) > GUIDANCE_MAX_BYTES:
        debug_log(
            f"extensibility: polyclaw-security-guidance.md combined size "
...
def _load_user_patterns(cwd: Optional[str]) -> List[Dict[str, Any]]:
    ...
    for label, path in _config_paths(cwd, "security-patterns"):
        ...
        # ".polyclaw/security-patterns" or ".polyclaw/security-patterns.local");
