#!/usr/bin/env python3
"""SessionStart bootstrap: ensure polyclaw_agent_sdk is importable for the
agentic commit reviewer.

If polyclaw_agent_sdk already imports in the current python3, this is a no-op.
Otherwise it creates a venv at ~/.polyclaw/security/agent-sdk-venv and installs
...
The venv lives under ~/.polyclaw/security/ (same dir the plugin already uses
...
NOOP_SYSTEM = 0      # polyclaw_agent_sdk already importable in system python
...
def _sdk_on_syspath() -> bool:
    ...
    try:
        return importlib.util.find_spec("polyclaw_agent_sdk") is not None
    except Exception:
        return False
...
def _plugin_version_int() -> int:
    ...
    try:
        p = Path(__file__).parent.parent / ".polyclaw-plugin" / "plugin.json"
...
def main() -> tuple[int, str, str]:
    ...
    if venv_py.exists():
        try:
            r = subprocess.run(
                [str(venv_py), "-c", "import polyclaw_agent_sdk"],
    ...
    try:
        ...
        subprocess.run(
            [str(venv_py), "-m", "pip", "install", "--quiet",
             "--disable-pip-version-check", "polyclaw-agent-sdk"],
