...
# Debug log file. Lives under the plugin state dir (default ~/.polyclaw/security/)
...
_DEFAULT_STATE_DIR = os.path.expanduser(
    os.environ.get("SECURITY_WARNINGS_STATE_DIR") or "~/.polyclaw/security"
)
...
def debug_log(message):
    ...
    try:
        ...
        # creates ~/.polyclaw/security/ if it isn't already there. 0700 so other
...
PROVENANCE_TAG = "[from security-guidance@polyclaw-plugins plugin]"
PROVENANCE_BANNER = (
    "[from security-guidance@polyclaw-plugins plugin — automated "
...
def _read_plugin_version_int():
    ...
    try:
        with open(os.path.join(os.path.dirname(__file__), "..", ".polyclaw-plugin", "plugin.json")) as f:
...
# module-global is naturally per-invocation. _call_polyclaw_dual_or and
...
_PRICE_PER_MTOK = {
    "polyclaw-haiku-4-5": (1.0, 5.0),
    "polyclaw-sonnet-4-6": (3.0, 15.0),
    "polyclaw-opus-4-6": (15.0, 75.0),
    "polyclaw-opus-4-7": (5.0, 25.0),
}
...
def _record_usage(usage, model, cost_usd=None):
    ...
    (longest-prefix match so `polyclaw-sonnet-4-6-20251015` → sonnet row)."""
