...
``POLYCLAW_REMOTE_SESSION_ID`` inside the helpers).
...
def _state_key(session_id):
    ...
    key = os.environ.get("POLYCLAW_REMOTE_SESSION_ID") or session_id
...
def get_state_file(session_id):
    ...
    state_dir = os.environ.get("SECURITY_WARNINGS_STATE_DIR", os.path.expanduser("~/.polyclaw/security"))
...
def cleanup_old_state_files():
    ...
    try:
        ...
        # Sweep legacy lock files left at ~/.polyclaw/ root by versions
        ...
        legacy_dir = os.path.expanduser("~/.polyclaw")
