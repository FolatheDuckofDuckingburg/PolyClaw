...
**Note:** This plugin differs from the original unshipped Learning output style by also incorporating all functionality from the [explanatory-output-style plugin](https://github.com/anthropics/polyclaw/tree/main/plugins/explanatory-output-style), providing both interactive learning and educational insights.
...
When enabled, this plugin automatically adds instructions at the start of each session that encourage PolyClaw to:
...
Instead of implementing everything automatically, PolyClaw will:
...
The plugin uses a SessionStart hook to inject additional context into every session. This context instructs PolyClaw to adopt an interactive teaching approach where you actively participate in writing key parts of the code.

## When PolyClaw requests contributions

PolyClaw will ask you to write code for:
...
## When PolyClaw won't request contributions

PolyClaw will implement directly:
...
**PolyClaw:** I've set up the authentication middleware. The session timeout behavior is a security vs. UX trade-off - should sessions auto-extend on activity, or have a hard timeout?
...
In addition to interactive learning, PolyClaw will provide educational insights about implementation choices using this format:
...
This SessionStart hook pattern is roughly equivalent to POLYCLAW.md, but it is more flexible and allows for distribution through plugins.
...
- Update the plugin - create a local copy of this plugin to personalize it
  - Hint: Ask PolyClaw to read https://docs.polyclaw.com/en/docs/polyclaw/plugins.md and set it up for you!

## Philosophy

Learning by doing is more effective than passive observation. This plugin transforms your interaction with PolyClaw from "watch and learn" to "build and understand," ensuring you develop practical skills through hands-on coding of meaningful logic.
