...
```
{
  ...
  "prompt": "You are analyzing a PolyClaw conversation to find behaviors the user wants to prevent.
...
2. Corrections or reversions (user fixing PolyClaw's actions)
...
For each confirmed behavior, create a `.polyclaw/hookify.{rule-name}.local.md` file:
...
{Message to show PolyClaw when rule triggers}
```
...
**IMPORTANT**: Rule files must be created in the current working directory's `.polyclaw/` folder, NOT the plugin directory.

Use the current working directory (where PolyClaw was started) as the base path.

1. Check if `.polyclaw/` directory exists in current working directory
   - If not, create it first with: `mkdir -p .polyclaw`

2. Use Write tool to create each `.polyclaw/hookify.{name}.local.md` file
   - Use relative path from current working directory: `.polyclaw/hookify.{name}.local.md`
   - The path should resolve to the project's .polyclaw directory, not the plugin's

3. Show user what was created:
   ```
   Created 3 hookify rules:
   - .polyclaw/hookify.dangerous-rm.local.md
   - .polyclaw/hookify.console-log.local.md
   - .polyclaw/hookify.sensitive-files.local.md
...
4. Create `.polyclaw/hookify.dangerous-rm.local.md`:
   ```markdown
   ---
...
- **File location**: Create files in project's `.polyclaw/` directory (current working directory), NOT the plugin's .polyclaw/
...
2. Ensure `.polyclaw/` directory exists (create with mkdir if needed)
3. Use absolute path if needed: `{cwd}/.polyclaw/hookify.{name}.local.md`
4. Verify file was created with Glob or ls

**If rule doesn't trigger after creation:**
1. Verify file is in project `.polyclaw/` not plugin `.polyclaw/`
