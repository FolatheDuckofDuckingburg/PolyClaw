...
1. Use Glob tool to find all hookify rule files:
   ```
   pattern: ".polyclaw/hookify.*.local.md"
   ```
...
**Status**: ✅ Active
**File**: .polyclaw/hookify.dangerous-rm.local.md
```
...
2. Or manually create `.polyclaw/hookify.my-rule.local.md` files
...
Check `${POLYCLAW_PLUGIN_ROOT}/examples/` for example rule files.
```
