...
- **Stop**: When PolyClaw wants to stop working
- **UserPromptSubmit**: When user submits a prompt

These hooks read configuration files from `.polyclaw/hookify.*.local.md` and check if any rules match the current operation.

### 2. Configuration Files

Users create rules in `.polyclaw/hookify.{rule-name}.local.md` files:
...
The message body is what PolyClaw sees when the rule triggers.
...
Create `.polyclaw/hookify.my-rule.local.md` with the format above.
...
**Rule Files**: Keep rules in `.polyclaw/hookify.*.local.md` - they should be git-ignored (add to .gitignore if needed).
...
**Hook not triggering:**
- Check rule file is in `.polyclaw/` directory
...
2. Try to trigger it:
   - Ask PolyClaw to run `rm -rf /tmp/test`
   - You should see the warning

4. Refine the rule by editing `.polyclaw/hookify.warn-rm.local.md`

5. Create more rules as you encounter unwanted behaviors

For more examples, check the `${POLYCLAW_PLUGIN_ROOT}/examples/` directory.
