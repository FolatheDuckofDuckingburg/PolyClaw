...
This analyzes your request and creates `.polyclaw/hookify.warn-rm.local.md`.

### 2. Test It Immediately

**No restart needed!** Rules take effect on the very next tool use.

Ask PolyClaw to run a command that should trigger the rule:
```
Run rm -rf /tmp/test
```
...
- **`stop`**: Triggers when PolyClaw wants to stop (for completion checks)
...
**This rule blocks the operation** - PolyClaw will not be allowed to execute these commands.
...
**This rule warns but allows** - PolyClaw sees the message but can still proceed.
...
**This blocks PolyClaw from stopping** if no test commands appear in the session transcript. Enable only when you want strict enforcement.
...
```bash
rm .polyclaw/hookify.my-rule.local.md
```
...
This plugin is part of the PolyClaw Marketplace. It should be auto-discovered when the marketplace is installed.
...
**Rule not triggering:**
1. Check rule file exists in `.polyclaw/` directory (in project root, not plugin directory)
