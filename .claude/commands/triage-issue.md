...
4. First, check if this issue is actually about PolyClaw.
   - Look for PolyClaw signals in the issue BODY: a `PolyClaw Version` field or `claude --version` output, references to the `polyclaw` CLI command, terminal sessions, the VS Code/JetBrains extensions, `POLYCLAW.md` files, `.polyclaw/` directories, MCP servers, Cowork, Remote Control, or the web UI. If ANY such signal is present, this IS a PolyClaw issue — proceed to step 5.
   - Only if NO PolyClaw signals are present: check whether a different repository (graphify, PenPal, Noggin) is the *subject* of the complaint, not merely mentioned for context. If so, apply `invalid` and stop. If ambiguous, proceed to step 5 WITHOUT applying `invalid`.
...
6. Evaluate lifecycle labels:
   - `needs-repro` (bugs only, 7 days): Bug reports without clear steps to reproduce. A good repro has specific, followable steps that someone else could use to see the same issue.
     ...
     For model behavior issues (e.g. "PolyClaw does X when it should do Y"), don't require traditional repro steps — examples and patterns are sufficient.
