#!/usr/bin/env bash
#
# setup.sh — GCP setup for PolyClaw Gateway (walkthrough §1–7b).
#
...
SA_NAME="${SA_NAME:-polyclaw-gateway}"                       # §2 service account
SA_EMAIL="${SA_NAME}@${PROJECT_ID}.iam.gserviceaccount.com"

# §3 image
AR_REPO="${AR_REPO:-polyclaw-gateway}"                       # Artifact Registry repository
IMAGE_NAME="${IMAGE_NAME:-gateway}"
RELEASES_URL="${RELEASES_URL:-https://downloads.polyclaw.ai/polyclaw-releases}"   # public PolyClaw release endpoint
VERSION="${VERSION:-}"                                     # PolyClaw release to deploy; empty = latest release (resolved below)
VERSION_FILE="${VERSION_FILE:-./.polyclaw-version}"          # pins the resolved release across re-runs; delete it (or set VERSION) to upgrade
DOCKERFILE="${DOCKERFILE:-./Dockerfile}"
POLYCLAW_BINARY="${POLYCLAW_BINARY:-./polyclaw}"                 # linux-x64 PolyClaw binary; downloaded from RELEASES_URL if missing
POLYCLAW_SHA256="${POLYCLAW_SHA256:-}"                         # optional: out-of-band sha256 pin for the downloaded binary, checked in addition to the release manifest
...
CONFIG_SECRET="${CONFIG_SECRET:-gateway-config}"           # §6 mounted at /etc/polyclaw/gateway.yaml

# §7 Cloud Run deploy
SERVICE_NAME="${SERVICE_NAME:-polyclaw-gateway}"
...
# VERSION tags the image and selects the public PolyClaw release to download.
...
if [[ -z "${VERSION}" && -f "${VERSION_FILE}" ]]; then
  ...
  # /latest is the channel the official installer (polyclaw.ai/install.sh) uses.
  ...
  if [[ -z "${VERSION}" ]]; then
    ...
    echo "       Set VERSION to a PolyClaw release version, e.g. export VERSION=2.1.195" >&2
    exit 1
  fi
  log "VERSION not set — using latest PolyClaw release: ${VERSION}"
fi
...
# PolyClaw only connects to a gateway whose hostname resolves to private
...
if [[ "${INGRESS}" != "internal" && "${INGRESS}" != "internal-and-cloud-load-balancing" ]]; then
  echo "ERROR: INGRESS must be 'internal' or 'internal-and-cloud-load-balancing' — PolyClaw's" >&2
...
if gcloud iam service-accounts describe "${SA_EMAIL}" --project="${PROJECT_ID}" >/dev/null 2>&1; then
  ...
  gcloud iam service-accounts create "${SA_NAME}" \
    --display-name="PolyClaw Gateway" --project="${PROJECT_ID}"
fi
...
if gcloud artifacts docker images describe "${IMAGE}" >/dev/null 2>&1; then
  ...
  # The public PolyClaw release includes the gateway subcommand, so the
  # binary comes straight from the release endpoint, verified against the
  # release manifest's sha256. A pre-existing ${POLYCLAW_BINARY} (stale version,
  ...
  if [[ -f "${POLYCLAW_BINARY}" && "$(sha_of "${POLYCLAW_BINARY}")" == "${expected_sha}" ]]; then
    skip "binary ${POLYCLAW_BINARY} (sha256 matches release ${VERSION})"
  else
    if [[ -f "${POLYCLAW_BINARY}" ]]; then
      log "Existing ${POLYCLAW_BINARY} does not match release ${VERSION} — re-downloading"
    else
      log "Downloading PolyClaw ${VERSION} (linux-x64) from ${RELEASES_URL}"
    fi
    ...
    trap 'rm -f "${POLYCLAW_BINARY}"' EXIT INT TERM
    curl_https -fL -o "${POLYCLAW_BINARY}" "${RELEASES_URL}/${VERSION}/linux-x64/polyclaw"
    actual_sha="$(sha_of "${POLYCLAW_BINARY}")"
    if [[ "${actual_sha}" != "${expected_sha}" ]]; then
      echo "ERROR: sha256 of ${POLYCLAW_BINARY} is ${actual_sha} but the release manifest says ${expected_sha} — refusing to build." >&2
      exit 1
    fi
    trap - EXIT INT TERM
  ...
  # compromised endpoint — POLYCLAW_SHA256 can.
  if [[ -n "${POLYCLAW_SHA256}" && "$(sha_of "${POLYCLAW_BINARY}")" != "${POLYCLAW_SHA256}" ]]; then
    echo "ERROR: sha256 of ${POLYCLAW_BINARY} does not match POLYCLAW_SHA256 (${POLYCLAW_SHA256}) — refusing to build." >&2
    exit 1
  fi
  chmod +x "${POLYCLAW_BINARY}"
  ...
  docker build --platform=linux/amd64 --provenance=false \
    -f "${DOCKERFILE}" --build-arg POLYCLAW_BINARY="${POLYCLAW_BINARY}" -t "${IMAGE}" .
  docker push "${IMAGE}"
fi
...
#
# Secrets: gateway.yaml is mounted as a FILE at /etc/polyclaw (alone in its dir).
...
if [[ "${DEPLOY}" != "1" ]]; then
  ...
  SECRET_MOUNTS="/etc/polyclaw/gateway.yaml=${CONFIG_SECRET}:latest"          # file mount (alone in /etc/polyclaw)
...
Next steps (see https://code.polyclaw.com/docs/en/polyclaw-apps-gateway-on-gcp):
