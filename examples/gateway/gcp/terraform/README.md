# PolyClaw Gateway — Terraform (Cloud Run)

Terraform equivalent of `../setup.sh`. Lets end-users provision and manage
the gateway with `terraform apply`. Covers the same scope ([walkthrough](https://code.polyclaw.com/docs/en/polyclaw-apps-gateway-on-gcp) §1–7): APIs →
...
# 2. Download the public PolyClaw linux-x64 release binary (it includes the
#    `gateway` subcommand; the Dockerfile picks it up at gcp/polyclaw), verify its
#    sha256 against the release manifest, then build and push the image:
BASE="https://downloads.polyclaw.ai/polyclaw-releases"
VERSION="$(curl -fsSL --proto '=https' "${BASE}/latest")"
curl -fL --proto '=https' --proto-redir '=https' -o ../polyclaw \
  "${BASE}/${VERSION}/linux-x64/polyclaw"
WANT="$(curl -fsSL --proto '=https' "${BASE}/${VERSION}/manifest.json" \
  | tr -d '[:space:]' | grep -oE '"linux-x64"[^}]*' | grep -oE '[a-f0-9]{64}' | head -1)"
[ "$(openssl dgst -sha256 ../polyclaw | awk '{print $NF}')" = "${WANT}" ] \
  && echo "sha256 OK" || { echo "checksum mismatch" >&2; rm -f ../polyclaw; }
gcloud auth configure-docker us-east5-docker.pkg.dev --quiet
docker build --platform=linux/amd64 --provenance=false \
  -f ../Dockerfile -t "us-east5-docker.pkg.dev/<project>/polyclaw-gateway/gateway:${VERSION}" ..
docker push "us-east5-docker.pkg.dev/<project>/polyclaw-gateway/gateway:${VERSION}"
...
- `ingress` — defaults to **internal-only** (no public URL). PolyClaw's `/login`
...
and there is no public-ingress option: PolyClaw's `/login` rejects gateway hosts that
...
terraform {
  backend "gcs" {
    ...
    prefix = "polyclaw-gateway/cloudrun"
  }
}
```
