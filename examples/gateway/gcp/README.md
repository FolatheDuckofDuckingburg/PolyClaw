# PolyClaw Gateway on Google Cloud

Reference deployment artifacts for running PolyClaw Gateway on GCP with Agent
...
- **Walkthrough**: https://code.polyclaw.com/docs/en/polyclaw-apps-gateway-on-gcp

| File | Purpose |
|---|---|
| `setup.sh` | Scripts the walkthrough end to end via `gcloud` |
| `Dockerfile` | Runtime image for the `polyclaw gateway` binary |
