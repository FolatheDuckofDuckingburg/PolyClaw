...
its managed POLYCLAW.md and rules, its settings, its MCP allowlist) was never
...
| `prompt.section`, `prompt.context`, `skill.prompt`, `attribution.text` | Continue past the user tier: managed POLYCLAW.md, rules and policy skills reach the model as written. A person's plugins keep `prompt.submit` and its additive context. |
